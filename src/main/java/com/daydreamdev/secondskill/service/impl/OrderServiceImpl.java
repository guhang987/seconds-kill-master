...
public class OrderServiceImpl implements OrderService {
    ...
    public int createWrongOrder(int sid) throws Exception {
        Stock stock = checkStock(sid); // 从数据库查询商品数量
        saleStock(stock); // 修改商品数量
        int res = createOrder(stock); // 创建订单记录
    ...
    public int createOptimisticOrder(int sid) throws Exception {
        Stock stock = checkStock(sid); // 校验库存
        saleStockOptimstic(stock);       // 乐观锁更新
        int res = createOrder(stock);  // 创建订单记录
    ...
    public void createOrderWithLimitAndRedisAndKafka(int sid) throws Exception {
        ...
        kafkaTemplate.send(kafkaTopic, gson.toJson(stock)); // 发送到kafka集群中的每一条消息都要指定一个主题
